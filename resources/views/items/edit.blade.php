...
@section('content')
    ...
    <form action="{{ route('items.update', ['item' => $item->id]) }}" method="POST">
        ...
        <div class="col-md-6 mb-3"> <label for="nama">Nama</label> <input type="text"
                ...
                value="{{ old('nama') ?? $item->nama }}"> @error('nama')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>

        <div class="col-md-6 mb-3"> <label for="harga">Harga</label> <input type="number"
                class="form-control @error('harga') is-invalid @enderror" name="harga" id="harga"
                value="{{ old('harga') ?? $item->harga }}"> @error('harga')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-6 mb-3"> <label for="stok">Stok</label> <input type="number"
                ...
                value="{{ old('stok') ?? $item->stok }}"> @error('stok')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
